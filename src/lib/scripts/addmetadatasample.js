...
const oldFile = "BrassiToL_natives.json";
...
async function updateMetadata() {
	try {
		...
		const oldDataMap = new Map(
			oldData.map((item) => [item.SAMPLE, item.WCVP_WGSRPD_LEVEL_3_native])
		);

		// Update metadata with WCVP_WGSRPD_LEVEL_3_native from oldData
		metadata.forEach((item) => {
			if (oldDataMap.has(item.SAMPLE)) {
				item.WCVP_WGSRPD_LEVEL_3_native = oldDataMap.get(item.SAMPLE);
			}
		});
